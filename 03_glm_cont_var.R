...
glm_function_cont <- function(occurrence,
  ...
  contribution <- pbmcapply::pbmclapply(1:length(species_name), function(j){
    ...
    fitting <- raw_occurrence$fitting[,c("id_spygen", species_name[j])] #attention ici on prend fitting pour voir effet variables !!!!!!
    ...
    vip.25_glm <- DALEX::model_parts(explainer = explainer_glm, 
                                     ...
                                     predicted = explainer_glm[["model"]][["predicted"]], #explainer_glm$y_hat
